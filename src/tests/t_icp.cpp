...
BOOST_AUTO_TEST_CASE( testAll )
{
	...
	ICP<DIM,Point>::TQuat q = icp.getRot();
	ICP<DIM,Point>::TVec  v = icp.getTrans();
	for(int i=0; i<S-1; i++){ // ignore last=outlier
		Point x = query_orig[i];
		Point y = model[i];
		ICP<DIM,Point>::TQuat qx(0,x[0],x[1],x[2]);
		ICP<DIM,Point>::TQuat qy(0,y[0],y[1],y[2]);
		ICP<DIM,Point>::TQuat qv(0,v[0],v[1],v[2]);
		ICP<DIM,Point>::TQuat qxtrans = q * qx * boost::math::conj(q) + qv;
...
BOOST_AUTO_TEST_CASE( test2 )
{
	...
	// modify model a bit
	ICP<DIM,Point>::VectorRotator    rotate;
	ICP<DIM,Point>::VectorTranslator translate;
	ICP<DIM,Point>::TQuat qr(1,0.05,0.00,0.00);
